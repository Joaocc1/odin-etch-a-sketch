...
4. It was on this step that I realized my mistake in the second step and proceeded to correct it which took several tries to get it right. In my initial tries the height and width would not be the same even though it fill the whole area or it would be the same but would also have gaps between some of the `<div>` elements. Eventually I settled on using percentages to define the size of the pixels which worked perfectly with whatever number was used to defibe the number of pixels. At this stage I also changed the way the "hover" effect worked by no longer adding a class but instead change it's opacity directly.
